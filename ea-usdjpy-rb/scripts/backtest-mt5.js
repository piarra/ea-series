...
const SOURCE_FILE = process.env.MQ5_SOURCE || path.join(ROOT_DIR, "USDJPY_RB.mq5");
...
const PERIOD = process.env.BT_PERIOD || "M1";
const MODEL = process.env.BT_MODEL || "1";
...
function collectTesterEvidence(terminalDataDir, symbol) {
  const raw = powershell(
    `$dir=${psQuote(
      ...
      symbol
    )});$log=Get-ChildItem -Path $dir -Filter '*.log' -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1;if($log){Write-Output ('TESTER_LOG:'+ $log.FullName);$tail=Get-Content -Path $log.FullName -Tail 3000;$missing=$tail | Select-String -Pattern ('symbol ' + $sym + ' (does not exist|not exist)') | Select-Object -Last 1;if($missing){Write-Output ('TESTER_MISSING:' + $missing.Line)};$summary=$tail | Select-String -Pattern ('\\[RB_STATS\\]|final balance|' + $sym + ',M[0-9]+: .*Test passed in|' + $sym + ',M[0-9]+: total time from login to stop testing|last test passed') | Select-Object -Last 8;$summary | ForEach-Object { 'TESTER_SUMMARY:' + $_.Line }}`,
...
function main() {
  ...
  runChecked(
    "node",
    [path.join(ROOT_DIR, "..", "scripts", "build-mt5.js"), "--source", SOURCE_FILE],
    "Pre-backtest build"
  );
  ...
  for (const symbol of candidates) {
    ...
    const fromSlug = FROM_DATE.replace(/\./g, "");
    const toSlug = TO_DATE.replace(/\./g, "");
    const reportPrefix = `${EA_NAME}-backtest-${fromSlug}-${toSlug}-${slug}`;
