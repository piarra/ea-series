...
- 方向別にバスケット決済を行う。
- 日本時間 06:30-08:15 は取引停止 (新規ポジションは建てない)。
...
- 段数上限は `MaxLevels`。
  ...
  - 停止中に価格が大きく進んでも、段数はスキップしない。
    ナンピン再開時は、未約定の次レベルから順番に追加する。
...
- 取引停止時間中は、新規エントリー(初期/再エントリー)を停止し、
  既存バスケットのナンピンと利確は継続する。
  さらに `バスケットPL >= -0.5 * ProfitBase` で強制クローズ。
- SafetyMode:
